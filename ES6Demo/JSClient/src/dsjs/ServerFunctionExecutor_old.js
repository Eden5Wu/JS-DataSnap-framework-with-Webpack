...
function ServerFunctionExecutor(className, connectionInfo, owner)
{
  ...
  this.executeMethodURL = function(url, contentParam, requestType, callback, hasResult, accept) {
    ...
    {
      contentParam = JSON.stringify(contentParam) // contentParam.toJSONString();
    }
   ...
   *         otherwise been passed to the callback
   */
     this.fetchMethodURL = async function(url, contentParam, requestType, callback, hasResult, accept) {
     ...
     *         otherwise been passed to the callback
     */
    this.fetchMethod = function(methodName, requestType, params, callback, hasResult, requestFilters, accept) {
      var url = this.getMethodURL(methodName, requestType, params, requestFilters);    
      return this.fetchMethodURL(url[0], url[1], requestType, callback, hasResult, accept);
    };
...
function parseHTTPResponse(request)
{
  ...
  if (request != null && request.responseText != null)
  {
    ...
    if (responseText.length > 0 )
    {
      ...
      try
      {
        JSONResultWrapper = JSON.parse(responseText); //responseText.parseJSON();
      }
      catch(e)
      {
        JSONResultWrapper = responseText;
 ...
 */
 function parseFetchSessionID(response)
 {
   if (response != null)
   {
     //pragma may store the Session ID value to use in future calls
