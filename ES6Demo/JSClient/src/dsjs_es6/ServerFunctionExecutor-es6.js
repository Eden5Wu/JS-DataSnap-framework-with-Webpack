...
var $$USE_SESSION_COOKIE$$ = false;
var $$SESSION_COOKIE_PREFIX$$ = "";
initSessionData(false);

function notifySessionExpired(oldSessionID){
    alert('Please re-login, session ['+oldSessionID+'] is expired!')
}
...
function getAuth(userName, password, alertOnFailure) {
    let authStr = window.btoa(userName + ":" + password)
 ...
 */
export function ServerFunctionExecutor(className, connectionInfo, owner) {
...
/**
 * Checks if the given Function is Reference.
 * @param the Function
 * @return true if the Function is Reference, false otherwise
 */
function isReferenceAFunction(ref) {
    return ref != null && typeof ref == 'function';
}

export class DSFunctionExecutor extends ServerFunctionExecutor {
