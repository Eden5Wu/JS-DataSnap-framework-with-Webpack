...
function executeServerFunction(formId, className, methodName)
{
  ...
  if (AdminInst != null)
  {
	...
	var connectionInfo = JSON.stringify(AdminInst.connectionInfo); //AdminInst.connectionInfo.toJSONString();
...
function updateResultFrame(className, methodName, resultObject)
{
  ...
  if (callCell != null && resultCell != null)
  {
    ...
    else
    {
      ...
      var resultString = resultObject == null ? "[no result]" : JSON.stringify(resultObject); //resultObject.toJSONString();
    }
